...
class Index:
    ...
    def indexing(self, m, l):
        ...
        for word in m.split():
            ...
            try:
                self.index[cleaned_word].append(l)
            except KeyError:
                self.index[cleaned_word]=[l]
...
class PIndex(Index):
    ...
    def load_poems(self):
        ...
        # IMPLEMENTATION
        # ---- start your code ---- #
        with open('AllSonnets.txt','r') as f:
    ...
    def get_poem(self, p):
        ...
        # IMPLEMENTATION
        # ---- start your code ---- #
        roman_numeral = self.int2roman.get(p, [])
        target_title = roman_numeral + '.'
        if target_title in self.index:
            start_line_at = self.index[target_title][0]
        else:
            return []
        next_roman_numeral = self.int2roman.get(p + 1)
        if next_roman_numeral:
            next_target_title = next_roman_numeral + '.'
            if next_target_title in self.index:
                end_line_at = self.index[next_target_title][0]
        else: 
            end_line_at = self.total_msgs
        poem = self.msgs[start_line_at:end_line_at]
